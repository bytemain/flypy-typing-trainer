...
(() => {
  ...
  function handleCopyKeydown(e) {
    ...
    // Let focused form fields (for example the search input) handle editing keys.
  ...
  function renderItem(item) { const full=item.fullCode || (item.soundCode ? item.soundCode + item.shapeCode : `??${item.shapeCode}`); const src=item.sources[0] ? item.sources[0].replace(/^.*\//,'') : '手动/未知来源'; const decomp=getDecomp(item); const shapeParts=decomp.structure?`<div class="result-meta">形码 <code>${escapeHtml(decomp.structure)}</code>${decomp.components && decomp.components!==decomp.structure?` · 完整拆分 <code>${escapeHtml(decomp.components)}</code>`:''}</div>`:''; return `<div class="result-item" title="${escapeHtml(item.sources.join('\n'))}"><div class="result-char">${escapeHtml(item.char)}</div><div><div class="result-code">${escapeHtml(full)} <span class="result-meta">前两码 ${escapeHtml(item.soundCode||'—')} · 后两码 ${escapeHtml(item.shapeCode||'—')}</span>${item.common?'<span class="pill">常用</span>':''}</div>${shapeParts}<div class="result-meta">见 ${item.seen} · 对 ${item.correct} · 错 ${item.wrong} · ${escapeHtml(src)}</div></div><button class="ghost" type="button" onclick="window.__xhTrainerPick('${encodeURIComponent(item.key)}')">练</button></div>`; }
